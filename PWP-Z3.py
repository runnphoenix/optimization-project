...
Y_H_c = [(Ys[i] + Ys[i] <= H) for i in range(N)]
...
implied_X_c = [ Sum([ If(And(Xs[i]<=x, x<Xs[i]+Ws[i]), Hs[i], 0) for x in range(W) for i in range(N) ]) <= H ]
implied_Y_c = [ Sum([ If(And(Ys[j]<=y, y<Ys[j]+Hs[j]), Ws[j], 0) for y in range(H) for j in range(N) ]) <= W ]

PWP_c = Yi_c + Xi_c + X_W_c + Y_H_c + disjunctive_c #+ implied_X_c
